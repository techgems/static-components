...
<Aside type="caution" title="static-for owns type">
`static-for` always sets `type`, either from this property or by inference. That means `type` is
never carried by [attribute passthrough](/reference/static-attributes-tag-helper) — a `type` a
consumer writes on your component is ignored rather than applied and then silently overwritten.

To let consumers choose the type, declare a property for it on your component and write it into the
template yourself:
...
```razor
<input static-for="@Model.InputExpression" type="@Model.Type" class="…" />
```
</Aside>
