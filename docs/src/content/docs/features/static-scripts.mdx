...
|---|---|---|
| `render-once` | `object` | Deduplicates the script — renders it only once per page, keyed by the model's type. |
| `teleport-script` | `bool` | Moves the script to the `<render-static-scripts />` location instead of rendering inline. |
| `disable-render` | `bool` | Suppresses the script entirely when `true`. |
...
<Aside type="note">
Notice that having scripts in the middle of your HTML isn't ideal — they can get executed before the DOM is ready. You can use the `defer` attribute for that, but in the next section we will cover `teleport-script` as a more robust solution.
</Aside>
...
<Aside type="tip">
The `<render-static-scripts />` tag helper is a self-closing element included with Static Components. Place it in your layout — typically right before the closing `</body>` tag — and all teleported scripts will render there.
</Aside>
