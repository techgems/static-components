...
<Aside type="caution">
Overriding `ProcessAsync` will cause the default partial view rendering to **not happen**. You must call one of the `RenderPartialView` helper methods yourself if you still want the Razor template to render.
</Aside>
...
|---|---|---|
| `output.Content` | The rendered partial view HTML | The actual component output. |
| `output.TagName` | `null` | Removes the wrapper tag so only the partial's HTML is emitted. |
...
<Aside type="note">
To override `ProcessAsync` properly you require knowledge of how to write your own [Tag Helpers](https://learn.microsoft.com/en-us/aspnet/core/mvc/views/tag-helpers/intro?view=aspnetcore-10.0) in ASP.NET Core. By overriding it you can take complete control of the way Razor gets processed and how the output gets handled.
</Aside>
