// Packages the Claude Skill in `skill/static-components/` into `public/static-components.skill`
// so the "Using AI" docs page can serve it as a download.
//
// The skill's markdown files are the source of truth and live in the repository; the archive is a
...
//
// The package version in the skill is written as `{{PACKAGE_VERSION}}` and substituted here from
// `docs/.env`, the same source the site reads — so the skill Claude installs always quotes the
// released version.
...
const skillName = 'static-components';
// Kept at the site root rather than under `downloads/` so the published `/static-components.skill`
// URL keeps working for anyone who already has the link.
const outputFile = join(docsRoot, 'public', `${skillName}.skill`);
...
function buildZip(files) {
	...
	for (const file of files) {
		...
		// archive extracts as `static-components/SKILL.md`.
